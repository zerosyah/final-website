...
const signin = async (req, res, next) => {
  ...
  try {
    ...
    if (!validPassword) return next(handleError(401, "invalid password or email"));
  ...
  try{
    ...
    if(user){
      ...
      const newUser = new userModel({
        ...
        firstName: req.body.name.split(" ").join("").toLowerCase() + Math.floor(Math.random() * 10000).toString(), email: req.body.email,
